/*
  ioports.c - driver code for Texas Instruments MSP432E401Y ARM processor
...
static void enable_irq (const input_signal_t *input, pin_irq_mode_t irq_mode)
{
    if(irq_mode == IRQ_Mode_None)
        ...
        GPIOIntTypeSet(input->port, input->bit, irq_mode == IRQ_Mode_Falling ? GPIO_FALLING_EDGE : (input->irq_mode == IRQ_Mode_Change ? GPIO_BOTH_EDGES : GPIO_RISING_EDGE));
...
void ioports_event (input_signal_t *input)
{
    ...
    if(input->interrupt_callback)
        input->interrupt_callback(ioports_map_reverse(&digital.in, get_port(input)), DIGITAL_IN(input->port, input->pin));
    else
        event_bits |= input->bit;
